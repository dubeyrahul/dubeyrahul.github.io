...
So, now when our tokenizer sees an unknown word while encoding, it splits it into tokens it has seen before. For e.g. if we are trying to tokenize a word: "absobloodylutely", it might break it up into <abso>, <bloody>, <lutely> subword tokens (assuming these 3 are present in the vocabularly generated by BPE tokenizer). 

I'll dive deeper into BPE and sub-word tokenization in a separate blog post.
